    ...
    >
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <main className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden px-4 py-3">
          ...
          {/* TABLA SCADA: filas = rollos, columnas = variables */}
          <section className="min-h-0 flex-1 overflow-hidden rounded-xl border-2 border-slate-200 bg-white">
            {variablesParaMostrar.length === 0 || historial.length === 0 ? (
              <div className="flex h-full items-center justify-center text-sm font-semibold text-slate-400">
                {variablesParaMostrar.length === 0
                  ? "No hay variables activas para mostrar."
                  : "Aún no hay rollos capturados en este turno."}
              </div>
            ) : (
              <div className="flex h-full flex-col">
                <div className="flex shrink-0 items-center justify-between border-b border-slate-200 bg-slate-50 px-3 py-1.5">
                  <h2 className="text-[12px] font-black uppercase tracking-[0.22em] text-slate-600">
                    Rollos Capturados · Turno Actual
                  </h2>
                ...
                <div className="min-h-0 flex-1 overflow-hidden">
                  <table className="h-full w-full table-fixed border-collapse">
                    <thead>
                      <tr className="border-b-2 border-slate-300 bg-slate-100 text-slate-600">
                        <th
                          className="px-2 py-1 text-left text-[10px] font-black uppercase tracking-wider"
                          style={{ width: "58px" }}
                        >
                          Hora
                        </th>
                        ...
                        {variablesParaMostrar.map((v) => (
                          <th
                            key={v.clave}
                            className="px-1 py-1 text-center text-[10px] font-black uppercase tracking-wider"
                            title={v.etiqueta}
                          >
                            <div className="truncate">{v.etiqueta}</div>
                            {v.unidad && (
                              <div className="text-[9px] font-bold normal-case text-slate-400">
                                {v.unidad}
                              </div>
                            )}
                          </th>
                        ...
                        <th
                          className="px-2 py-1 text-center text-[10px] font-black uppercase tracking-wider"
                          style={{ width: "70px" }}
                        >
                          Estado
                        </th>
                    ...
                    <tbody>
                      {historial.map((h, idx) => {
                        const isLast = idx === 0; // historial está reverse → más reciente primero
                        const rowAccent =
                          h.status === "bad"
                            ? "bg-rose-50"
                            : h.status === "warn"
                        ...
                        return (
                          <tr
                            key={h.id}
                            className={`${rowAccent} ${lastBorder} border-b border-slate-100`}
                            style={{
                              fontSize: `${densidad.value}px`,
                              height: `${100 / historial.length}%`,
                            }}
                          >
                            <td className="px-2 font-mono font-semibold tabular-nums text-slate-500">
                              {h.hora}
                            </td>
                            <td className="px-2 font-mono font-extrabold tabular-nums text-slate-900 truncate">
                              {h.rollo}
                            </td>
                            {h.vars.map((v) => {
                              const vColor =
                              ...
                              return (
                                <td
                                  key={v.clave}
                                  className={`px-1 text-center font-mono tabular-nums ${vColor}`}
                                >
                                  {v.valor === null
                                    ? "—"
                                    : Number(v.valor).toFixed(DIGITS_DE_CLAVE[v.clave] ?? 1)}
                                </td>
                              );
                            })}
                            <td className="px-2 text-center">
                              <span
                                className={`inline-flex items-center gap-1 font-black uppercase tracking-wider ${estadoColor}`}
                              >
          ...
          {/* FRANJA INFERIOR: VarCards del ÚLTIMO rollo (live) */}
          <section className="shrink-0">
            {variablesParaMostrar.length === 0 ? (
              <div className="flex h-[160px] items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-white text-sm font-semibold text-slate-400">
