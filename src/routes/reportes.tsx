...
function ReporteNoConformeItem({ enabled }: { enabled: boolean }) {
  ...
  const periodo = `${MESES_RM[month - 1]} ${year}`;
  const totalNC = rows.filter((r) => r.estatus === "NO CONFORME").length;
  const totalCond = rows.filter((r) => r.estatus === "CONDICIONADO").length;
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              ...
              Día 1 desde el Turno 2 · solo turnos cerrados. Datos directos de la base productiva.
            </p>
            <p className="mt-1 text-[11px] text-muted-foreground">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
        <div className="text-[11px] text-muted-foreground">
          {query.isLoading
            ? "Cargando datos…"
            : query.error
              ? <span className="text-destructive">{(query.error as Error).message}</span>
              : data
                ? `${rows.length} registros · ${totalNC} NC · ${totalCond} condicionados`
                : "Sin datos disponibles"}
          {error && <span className="ml-2 text-destructive">· {error}</span>}
        </div>
        <div className="flex items-center gap-2">
