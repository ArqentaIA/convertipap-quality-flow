ALTER TABLE public.producto_skus_sap ADD COLUMN IF NOT EXISTS es_principal boolean NOT NULL DEFAULT false;

CREATE UNIQUE INDEX IF NOT EXISTS producto_skus_sap_un_principal_por_producto
  ON public.producto_skus_sap (producto_id) WHERE es_principal;
...
BEGIN
  ...
  -- Si el producto tiene un SKU marcado como principal, ese tiene prioridad;
  -- en caso contrario se toma el primero en orden alfabético (comportamiento previo).
  IF EXISTS (
