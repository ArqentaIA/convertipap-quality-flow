ALTER TABLE public.pesajes_cintas_lotes ALTER COLUMN muestra_calidad_id DROP NOT NULL;
ALTER TABLE public.pesajes_cintas_lotes ADD COLUMN IF NOT EXISTS es_manual boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  IF v_rollo IS NULL OR length(v_rollo) = 0 OR _conductor_id IS NULL OR _bobinadora_id IS NULL OR _idempotency IS NULL THEN
...
GRANT EXECUTE ON FUNCTION public.crear_lote_pesaje_cintas_manual(text, numeric, uuid, uuid, uuid) TO authenticated;