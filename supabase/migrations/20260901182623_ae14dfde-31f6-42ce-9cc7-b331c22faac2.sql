-- 1) Tabla catálogo SKU SAP por producto
CREATE TABLE public.producto_skus_sap (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  producto_id uuid NOT NULL REFERENCES public.productos(id) ON DELETE CASCADE,
  clave_sku_sap text NOT NULL,
  descripcion_sap text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (producto_id, clave_sku_sap)
);

GRANT SELECT ON public.producto_skus_sap TO authenticated;
GRANT ALL ON public.producto_skus_sap TO service_role;

ALTER TABLE public.producto_skus_sap ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Usuarios autenticados pueden consultar SKUs SAP"
  ON public.producto_skus_sap FOR SELECT
  TO authenticated
  USING (true);

-- 2) Columnas snapshot en muestras (históricos quedan en NULL, sin tocar)
ALTER TABLE public.muestras_calidad
  ADD COLUMN sku_sap text,
  ADD COLUMN descripcion_sap text;

-- 3) Llenado automático del SKU principal SAP solo para planta TLX
...
BEGIN
  ...
  -- SKU SAP principal: solo planta TLX, llenado automático sin interacción del operador
  IF EXISTS (
