-- 1. Versión de etiqueta por cinta
ALTER TABLE public.pesajes_cintas
  ADD COLUMN IF NOT EXISTS version_etiqueta integer NOT NULL DEFAULT 1;

-- 2. Ampliación del registro de impresiones (no se crea tabla paralela)
ALTER TABLE public.impresiones_etiquetas_cintas
  ADD COLUMN IF NOT EXISTS cinta_id uuid REFERENCES public.pesajes_cintas(id),
  ADD COLUMN IF NOT EXISTS numero_impresion integer NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS version_etiqueta integer,
  ADD COLUMN IF NOT EXISTS qr_contenido jsonb,
  ADD COLUMN IF NOT EXISTS total_uniones_cintas integer;

CREATE INDEX IF NOT EXISTS idx_imp_etq_cintas_cinta ON public.impresiones_etiquetas_cintas(cinta_id);

-- 3. corregir_cinta: incrementa versión de etiqueta en la cinta vigente
...
BEGIN
  ...
  VALUES (v_ant.lote_id, v_new_id, 'CINTA_CORREGIDA',
    jsonb_build_object('peso_real_kg', v_ant.peso_cinta_kg, 'uniones', v_ant.uniones, 'ancho_util', v_ant.ancho_util, 'posicion', v_ant.posicion, 'version_etiqueta', COALESCE(v_ant.version_etiqueta,1)),
    jsonb_build_object('peso_real_kg', _peso_cinta_kg, 'uniones', _uniones, 'ancho_util', _ancho_util, 'posicion', v_ant.posicion, 'version_etiqueta', COALESCE(v_ant.version_etiqueta,1)+1, 'idempotency_key', _idempotency),
    _motivo, v_uid);

  RETURN jsonb_build_object('cinta_id', v_new_id, 'posicion', v_ant.posicion, 'version_etiqueta', COALESCE(v_ant.version_etiqueta,1)+1);
END $function$;

-- 4. preparar_impresion_etiquetas: por lote o por cinta, con datos de origen y QR
...
BEGIN
  ...
  IF v_tipo = 'REIMPRESION' AND (_motivo IS NULL OR length(trim(_motivo)) < 5) THEN
    RAISE EXCEPTION 'Motivo obligatorio para reimpresión (mínimo 5 caracteres).' USING ERRCODE='22023';
  END IF;
