...
BEGIN
  ...
  -- observaciones_generales es NOT NULL: un valor vacío se guarda como '' y nunca como NULL.
  FOREACH v_clave IN ARRAY ARRAY['operador','jefe_maquina','analista','observaciones_generales','sku_sap'] LOOP
    IF _cambios ? v_clave THEN
      ...
      IF v_clave = 'observaciones_generales' THEN
        v_txt_new := coalesce(v_txt_new, '');
      END IF;
