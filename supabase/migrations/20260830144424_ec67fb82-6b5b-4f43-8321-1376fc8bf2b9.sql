...
AS $function$
DECLARE
  n public.numeracion_rollos%ROWTYPE;
  v_cand bigint;
  v_folio text;
  v_sug text;
  v_saltos int := 0;
BEGIN
  SELECT * INTO n FROM public.numeracion_rollos WHERE maquina_id = _maquina_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('configurada', false, 'activa', false);
  END IF;

  v_cand := n.proximo_numero;
  v_sug := CASE WHEN COALESCE(n.relleno_digitos,0) > 0
                THEN lpad(v_cand::text, n.relleno_digitos, '0')
                ELSE v_cand::text END || '-' || n.sufijo;

  v_folio := v_sug;
  WHILE EXISTS (SELECT 1 FROM public.muestras_calidad WHERE numero_rollo = v_folio) LOOP
    v_cand := v_cand + 1;
    v_saltos := v_saltos + 1;
    v_folio := CASE WHEN COALESCE(n.relleno_digitos,0) > 0
                    THEN lpad(v_cand::text, n.relleno_digitos, '0')
                    ELSE v_cand::text END || '-' || n.sufijo;
    IF v_saltos > 10000 THEN EXIT; END IF;
  END LOOP;
