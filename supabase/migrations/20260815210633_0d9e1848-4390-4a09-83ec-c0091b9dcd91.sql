...
BEGIN
  ...
  -- IDEMPOTENCIA (1ra comprobación, sin lock).
  ...
  -- IDEMPOTENCIA (2da comprobación, YA con el lock adquirido).
  -- Cubre la carrera exacta: otra sesión con la misma clave pudo haber
  -- confirmado (COMMIT) mientras esta esperaba el lock.
  SELECT id, numero_rollo INTO v_prev
    FROM public.muestras_calidad
   WHERE idempotency_key = _idempotency;
  IF FOUND THEN
  ...
  IF v_rec.maquina_id IS NOT NULL AND v_rec.activo AND now() >= v_rec.vigente_desde THEN
    v_auto := true;
...
REVOKE ALL ON FUNCTION public.crear_muestra_con_mediciones(jsonb, jsonb, uuid) FROM anon;
