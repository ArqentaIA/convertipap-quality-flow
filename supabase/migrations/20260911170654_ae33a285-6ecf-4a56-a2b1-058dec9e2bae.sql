CREATE TABLE public.qc_edicion_permisos (
  ...
  UNIQUE (email, maquina_codigo)
);

GRANT SELECT ON public.qc_edicion_permisos TO authenticated;
GRANT ALL ON public.qc_edicion_permisos TO service_role;
ALTER TABLE public.qc_edicion_permisos ENABLE ROW LEVEL SECURITY;
CREATE POLICY "qc_edicion_permisos_select" ON public.qc_edicion_permisos
  FOR SELECT TO authenticated USING (true);

CREATE TRIGGER trg_qc_edicion_permisos_updated_at
  BEFORE UPDATE ON public.qc_edicion_permisos
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

INSERT INTO public.qc_edicion_permisos (email, maquina_codigo) VALUES
  ('gcalidad@convertipap.site', 'MP-04'),
  ('gcalidad@convertipap.site', 'MP-05'),
  ('gcalidad@convertipap.site', 'MP-06'),
  ('gcalidad@convertipap.site', 'MP-07'),
  ('msramosixt@convertipap.site', 'MP-01');

CREATE TABLE public.qc_ediciones_rollo (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_qc_ediciones_rollo_muestra ON public.qc_ediciones_rollo(muestra_id, created_at DESC);

GRANT SELECT ON public.qc_ediciones_rollo TO authenticated;
GRANT ALL ON public.qc_ediciones_rollo TO service_role;
ALTER TABLE public.qc_ediciones_rollo ENABLE ROW LEVEL SECURITY;
CREATE POLICY "qc_ediciones_rollo_select" ON public.qc_ediciones_rollo
  FOR SELECT TO authenticated USING (true);
...
BEGIN
  ...
  v_expira := v_cap + interval '12 hours';
  IF now() > v_expira THEN
    RETURN jsonb_build_object('puede', false, 'motivo', 'La ventana de edición de 12 horas ya venció.', 'expira_at', v_expira, 'maquina', v_codigo);
  END IF;
...
BEGIN
  ...
  -- 2) Datos operativos
  FOREACH v_clave IN ARRAY ARRAY['operador','jefe_maquina','analista','observaciones_generales'] LOOP
...
REVOKE ALL ON FUNCTION public.qc_editar_rollo(uuid, jsonb, text) FROM public;
